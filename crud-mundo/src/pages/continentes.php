...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $id = (int) $_POST['id'];
        $check = mysqli_query($conexao, "SELECT id FROM paises WHERE continente_id = $id");
        if (mysqli_num_rows($check) === 0) {
            mysqli_query($conexao, "DELETE FROM continentes WHERE id = $id");
        } else {
            echo "<script>alert('Não é possível excluir: continente possui países vinculados.');</script>";
        }
    }
...
if (isset($_GET['edit'])) {
    $id = (int) $_GET['edit'];
    $result = mysqli_query($conexao, "SELECT * FROM continentes WHERE id = $id");
...
<body class="min-h-screen bg-linear-to-b from-mocha-base via-mocha-mantle to-mocha-crust font-sans text-mocha-text">
    <div class="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8 space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-3xl font-serif italic text-mocha-mauve sm:text-4xl">Continentes</h1>
            <a href="../../index.php"
        ...
        <form method="POST"
            ...
            <h2 class="text-xl font-serif italic text-mocha-mauve"><?= $editMode ? 'Editar' : 'Novo' ?> Continente</h2>
            <?php if ($editMode): ?>
        ...
        <div class="rounded-2xl border border-mocha-surface1 bg-mocha-surface0/70 p-8 backdrop-blur-sm overflow-x-auto">
            <h2 class="text-xl font-serif italic text-mocha-mauve mb-4">Lista de Continentes</h2>
            <input type="text" id="searchInput" placeholder="Buscar continente..."
            ...
            <table class="w-full text-sm text-left">
                <thead>
                    <tr class="text-mocha-subtext0 border-b border-mocha-surface1">
                        ...
                        <th class="py-3 px-4">Total de Países</th>
                ...
                <tbody>
                    ...
                    $result = mysqli_query($conexao, "
                        SELECT c.*, (SELECT COUNT(*) FROM paises WHERE continente_id = c.id) AS total_paises
                        FROM continentes c ORDER BY c.nome
                    ");
                    ...
                    ?>
                        <tr class="border-b border-mocha-surface1 hover:bg-mocha-surface0/50 transition-colors">
                            ...
                            <td class="py-3 px-4"><?= $row['total_paises'] ?></td>
                            <td class="py-3 px-4 flex gap-3">
                                <a href="?edit=<?= $row['id'] ?>"
                                    class="text-mocha-blue hover:text-mocha-sapphire transition-colors text-sm">Editar</a>
                                <form method="POST" class="inline" onsubmit="return confirm('Excluir continente?')">
